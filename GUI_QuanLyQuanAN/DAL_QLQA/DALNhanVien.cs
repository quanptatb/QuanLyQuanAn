    ...
    public class DALNhanVien
    {
        ...
        public bool UpdateNhanVien(NhanVien nhanVien)
        {
            string sql = @"UPDATE NhanVien SET 
                            HoTen = @HoTen, IdChucVu = @IdChucVu, CaLam = @CaLam, DiaChi = @DiaChi, 
            ...
            parameters[10] = new SqlParameter("@IdNhanVien", SqlDbType.NVarChar) { Value = nhanVien.IdNhanVien };
            return DBUtil.ExecuteNonQuery(sql, parameters);
        }

        public bool DeleteNhanVien(string idNhanVien)
        {
            string sql = "UPDATE NhanVien SET TrangThai = 0 WHERE IdNhanVien = @IdNhanVien";
